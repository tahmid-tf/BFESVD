...
<div id="app" class="container">
    ...
    <div>
        <form v-on:submit.prevent="add_district">
            ...
            <span><input type="text" name="siteScaleOther" placeholder="Other" style="margin: 5px 0 8px 0;"
                         v-model="other_1"></span> <br>
            <!-- site scale end -->
            ...
            <label style="font-weight: 600">TEEB Scale Code
              ...
              <span class="popuptext" id="myPopup17">Select appropriate code: 1 if Local; 2 if Sub-national; 3 if National;
                  4 if Multi-country. For national scale study sites, leave blank.</span>
            </span>
            </label>
            <br>
            ...
            <span><input type="text" name="legalStatusOther" placeholder="Other" style="margin: 5px 0 8px 0;"
                         v-model="other_2"></span>
            <br>
            <!-- Legal Status start -->
            ...
            <br>
            <input type="" name="" v-model="site_area"> <br>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            submit() {
                ...
                axios.post('{{ route('study.update', session('tracker')) }}', formData).then(el => {
                    if (el.status == 200) {
                        if ('{{auth()->user()->role == 'user'}}') {
                            if("{{ \App\Valuation::where('es_value_id','=',session('tracker'))->orderBy('id', 'desc')->count() > 0 }}"){
                                window.location.href = "{{ route('valuation.edit',session('tracker')) }}";
                            }else{
            ...
            axios.get("{{ route('get-user-data-json', session('tracker') ) }}").then(el => {
                console.log(el.data.study);

                this.district = JSON.parse(el.data.study.district);
                this.division = JSON.parse(el.data.study.division);

                // this.site_scale = el.data.study.site_scale;
                // this.other_1 = el.data.study.other_1;
                // this.other_2 = el.data.study.other_2;
                // this.teeb = el.data.study.teeb;
                // this.location_name = el.data.study.location_name;
                // this.legal_status = el.data.study.legal_status;

                // this.site_area = el.data.study.site_area;
                // this.site_unit = el.data.study.site_unit;
                // this.site_ha = el.data.study.site_ha;
                // this.site_condition_text = el.data.study.site_condition_text;
                // this.site_condition = el.data.study.site_condition;
                // this.latitude = el.data.study.latitude;
                // this.longitude = el.data.study.longitude;
                // this.comment = el.data.study.comment;

                this.comment = el.data.study.comment  == null ? this.comment = '' : this.comment = el.data.study.comment;
                this.longitude = el.data.study.longitude  == null ? this.longitude = '' : this.longitude = el.data.study.longitude;
                this.latitude = el.data.study.latitude  == null ? this.latitude = '' : this.latitude = el.data.study.latitude;
                this.site_condition = el.data.study.site_condition  == null ? this.site_condition = '' : this.site_condition = el.data.study.site_condition;

                this.site_condition_text = el.data.study.site_condition_text  == null ? this.site_condition_text = '' : this.site_condition_text = el.data.study.site_condition_text;

                this.site_ha = el.data.study.site_ha  == null ? this.site_ha = '' : this.site_ha = el.data.study.site_ha;
                this.site_unit = el.data.study.site_unit  == null ? this.site_unit = '' : this.site_unit = el.data.study.site_unit;
                this.site_area = el.data.study.site_area  == null ? this.site_area = '' : this.site_area = el.data.study.site_area;
                this.legal_status = el.data.study.legal_status  == null ? this.legal_status = '' : this.legal_status = el.data.study.legal_status;
                this.location_name = el.data.study.location_name  == null ? this.location_name = '' : this.location_name = el.data.study.location_name;
                this.teeb = el.data.study.teeb  == null ? this.teeb = '' : this.teeb = el.data.study.teeb;
                this.other_2 = el.data.study.other_2  == null ? this.other_2 = '' : this.other_2 = el.data.study.other_2;
                this.other_1 = el.data.study.other_1  == null ? this.other_1 = '' : this.other_1 = el.data.study.other_1;
                this.site_scale = el.data.study.site_scale  == null ? this.site_scale = '' : this.site_scale = el.data.study.site_scale;
