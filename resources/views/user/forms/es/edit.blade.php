        ...
        <div>
            ...
            <span class="popuptext" id="myPopup5">See "TEEB Services" worksheet. Note - be careful NOT to include "." or "-" in the code. Only use commas (,) to separate the codes. <a
                    href="{{ route('visitor.teeb_sub_es') }}" target="_blank">Teeb SubES Codes</a></span>
                        ...
                        <textarea v-model="comment" style="width: 300%"
                        ...
                        <div>
                            <div>
                                ...
                                <select name="" id="edit_subes_item" style="width: 177px">
                                    ...
                                    <option
                                        value="Maintenance of genetic diversity"
                                        v-if="es_class === 'supporting' || es_class === 'cultural' "
                                    >
                                        Maintenance of genetic diversity
                                    </option>
                                    <!-- ----------------------- supporting -------------------------- -->
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            submit() {
                ...
                axios.post('{{ route('es.update', session('tracker')) }}', formData).then(el => {
                    if (el.status == 200) {
                        if ('{{auth()->user()->role == 'user'}}') {
                            if ("{{ \App\Study::where('es_value_id','=',session('tracker'))->orderBy('id', 'desc')->count() > 0 }}") {
                                window.location.href = "{{ route('study.edit',session('tracker')) }}";
                            } else {
            ...
            edit() {
                ...
                this.fullData[edit_id_var].subes_quantity_unit = edit_subes_quantity_unit.value;
        ...
        mounted() {
            ...
            axios.get("{{ route('get-user-data-json', session('tracker') ) }}").then(el => {
                this.fullData = JSON.parse(el.data.es.es_bundle);
                this.comment = el.data.es.comment == null ? this.comment = '' : this.comment = el.data.es.comment;
                this.teeb = el.data.es.teeb == null ? this.teeb = '' : this.teeb = el.data.es.teeb;
                this.iucn = el.data.es.iucn == null ? this.iucn = '' : this.iucn = el.data.es.iucn;
                this.sub_es_text = el.data.es.sub_es_text == null ? this.sub_es_text = '' : this.sub_es_text = el.data.es.sub_es_text;
                this.es_text = el.data.es.es_text == null ? this.es_text = '' : this.es_text = el.data.es.es_text;

                this.es_class = el.data.es.es_class;
            });
