          ...
          /> </span
            ...
            >
                ...
                >Comment From Reviewer
                    <span class="popup" onclick="myFunction14()">
                            <i class="fas fa-info-circle"></i>
    ...
    var app = new Vue({
        ...
        methods: {
            submitData() {
                ...
                axios.post('{{ route('ecosystem.update', session('tracker')) }}', formData).then(el => {
                    if (el.status == 200) {
                        if ('{{auth()->user()->role == 'user'}}') {
                            if ("{{ \App\Es::where('es_value_id','=',session('tracker'))->orderBy('id', 'desc')->count() > 0 }}") {
                                window.location.href = "{{ route('es.edit',session('tracker')) }}";
                            } else {
        ...
        mounted() {
            axios.get("{{ route('get-user-data-json', session('tracker') ) }}").then(el => {
                this.formData.zone = JSON.parse(el.data.ecosystem.zone);

                this.formData.faoBiome = el.data.ecosystem.fao_biome == null ? this.formData.faoBiome = '' : this.formData.faoBiome = el.data.ecosystem.fao_biome;
                this.formData.ecoSystem = el.data.ecosystem.ecosystem == null ? this.formData.ecoSystem = '' : this.formData.ecoSystem = el.data.ecosystem.ecosystem;
                this.formData.teeb = el.data.ecosystem.teeb == null ? this.formData.teeb = '' : this.formData.teeb = el.data.ecosystem.teeb;
                this.formData.others = el.data.ecosystem.others == null ? this.formData.others = '' : this.formData.others = el.data.ecosystem.others;
                this.formData.zone_text = el.data.ecosystem.zone_text == null ? this.formData.zone_text = '' : this.formData.zone_text = el.data.ecosystem.zone_text;
                this.formData.comment = el.data.ecosystem.comment == null ? this.formData.comment = '' : this.formData.comment = el.data.ecosystem.comment;
